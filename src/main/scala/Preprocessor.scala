...
object Preprocessor {
    ...
    /**
      ...
      * @param marlNames the list of names corresponding to each marker
      * @param thresh in [0, 1]
      * @return an expression matrix and marker name array
      */
    def expressionFilter(m: Array[Array[Double]], marlNames: Array[String], thresh: Double): (Array[Array[Double]], Array[String]) = {
        val nSam = m.length
        val mt = m.transpose
        ...
        return ((idx map mt).transpose, idx map marlNames)
    }
